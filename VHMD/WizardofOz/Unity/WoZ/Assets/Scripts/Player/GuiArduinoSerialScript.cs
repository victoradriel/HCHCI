...
public class GuiArduinoSerialScript : MonoBehaviour{
	...
	void OnMouseDown() {
		...
		if(fsession == 0){
			fsession = 1;
			StartButtonTxt.GetComponent<GUIText>().text = "NEXT";
	...
	void FirstSessionToBelt(){
		...
		switch(patternKey){
		case 0:
			TitleS.GetComponent<GUIText>().color = Color.gray;
		...
		case 4:
			...
			break;
		case 5:
			TitleS.GetComponent<GUIText>().color = Color.white;
			TitleF.GetComponent<GUIText>().color = Color.gray;
			SendToArduino = SendToArduinoZero;
		...
		case 10:
			...
			enableDraw = 0; 
			status.GetComponent<Renderer>().material.color = Color.white;
			TitleF.GetComponent<GUIText>().color = Color.white;
			fsession = 2;
			_SerialPortBelt.Write(SendToArduinoZero, 0, SendToArduinoZero.Length);
			break;
		}
		...
		if(enableDraw == 1){			
			...
			switch(contCicle){
			case 1:		
				status.GetComponent<Renderer>().material.color = Color.gray;
