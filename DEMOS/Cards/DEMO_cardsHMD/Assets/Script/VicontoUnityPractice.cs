...
public class VicontoUnityPractice : MonoBehaviour {
    ...
    void Update() {
        ...
        if (Input.GetKey(KeyCode.V) || Input.GetButton("Jump")) {
            if (trialState == false) {
                trialState = true;
            } 
        }       
    ...
    private void GetPointDirFOV(int inputdir) {
        ...
        if (inputdir >= 0 && inputdir <= 4) {
            motorIndx++;
            if (motorIndx == 1) { fovnow = 1; if (fovbefore != fovnow) { DisplayDirectionFOV(7); fovbefore = fovnow; headdir = inputdir; iddle = 0; print("West"); } }     // West
            if (motorIndx == 2) { fovnow = 2; if (fovbefore != fovnow) { DisplayDirectionFOV(6); fovbefore = fovnow; headdir = inputdir; iddle = 0; print("NW"); } }     // NW
            if (motorIndx == 3) {
                if (FOVindex == -1) { fovnow = 3; if (fovbefore != fovnow) { DisplayDirectionFOV(5); fovbefore = fovnow; headdir = inputdir; iddle = 0; print("Nleft"); } } // Nleft
                if (FOVindex == 0) { fovnow = 4; if (fovbefore != fovnow) { DisplayDirectionFOV(4); fovbefore = fovnow; headdir = inputdir; iddle = 0; print("NORTH"); } }  // NORTH
                if (FOVindex == 1) { fovnow = 5; if (fovbefore != fovnow) { DisplayDirectionFOV(3); fovbefore = fovnow; headdir = inputdir; iddle = 0; print("Nright"); } }  // Nright
            }
            if (motorIndx == 4) { fovnow = 6; if (fovbefore != fovnow) { DisplayDirectionFOV(2); fovbefore = fovnow; headdir = inputdir; iddle = 0; print("NE"); } }     // NE
            if (motorIndx == 5) { fovnow = 7; if (fovbefore != fovnow) { DisplayDirectionFOV(1); fovbefore = fovnow; headdir = inputdir; iddle = 0; print("East"); } }     // East
        }
        else {
            fovnow = 0;
            if (fovbefore != fovnow) {
