...
public class UIWidget : UIRect
{
	...
	protected override void OnStart ()
	{
		...
		{
			Debug.LogError("Widgets and panels should not be on the same object! Widget must be a child of the panel. " + this.name, this);
		}
		else if (!Application.isPlaying && GetComponents<UIWidget>().Length > 1)
		{
			Debug.LogError("You should not place more than one widget on the same object. Weird stuff will happen! " + this.name, this);
		}
#endif
		CreatePanel();
	}
