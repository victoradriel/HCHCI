...
public class KinectModelControllerV2 : MonoBehaviour {
	...
	void Start () {
		...
		InvokeRepeating ("printBoneState",5.0f, 0.25f);
	}
	...
	public void printBoneState()
	{
		for( int ii = 0; ii < _bones.Length; ii++)
		{
			...
			if(_bones[ii] != null)
			{
				loadTest.Log ("<####> Kinect_Bones,"+ ii + ","+ 
				_bones[ii].transform.localRotation.x +","+
				_bones[ii].transform.localRotation.y +","+
				_bones[ii].transform.localRotation.z +","+
				_bones[ii].transform.localRotation.w +","+
					+ Time.fixedTime);
			}
		}
	}
