    ...
    void UpdatePhysics()
    {
        ...
        for (int r = 0; r < Joints.Count; r++)
        {
            ...
            if (solverOverride != -1 && solverOverride > 1)
                go.GetComponent<Rigidbody>().solverIterations = solverOverride;
            ...
            foreach (RopeAttachedObject ao in attachedObjects)
            {
                ...
                switch (ao.jointType)
                {
                    ...
                    case RopeAttachmentJointType.Hinge:
                        ...
                        ao.jointRef.connectedBody = Joints[ao.jointIndex].GetComponent<Rigidbody>();//ao.jointRef.connectedBody = ao.go.rigidbody;
                        break;
                }
            }
        }
        ...
        if (Application.isPlaying)
        {
            ...
            switch (rao.jointType)
            {
                ...
                case RopeAttachmentJointType.Hinge:
                    ...
                    (rao.jointRef as HingeJoint).axis = rao.hingeAxis;
                    rao.jointRef.connectedBody = Joints[rao.jointIndex].GetComponent<Rigidbody>();//ao.jointRef.connectedBody = ao.go.rigidbody;
